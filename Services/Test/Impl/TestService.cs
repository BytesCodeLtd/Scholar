...
namespace Scholar.Services
{
    public class TestService(
        ...
        ITenantProvider tenant) : ITestService
    {
        ...
        private readonly IRepository<TestSettings> _testSettings = testSettings;
        ...
        public async Task<List<PaperCard>> GetPapersAsync(string? search)
        {
            // The query filter scopes tests to the institute; a non-super user with
            // no institute simply sees none.
        ...
        public async Task<PaperBuilderViewModel?> BuildPaperFormAsync(int[] selectedTopicIds, int subjectId, int? instituteId)
        {
        ...
        public async Task<List<Question>> SearchQuestionsAsync(int[] topicIds, QuestionType type, QuestionCategory[] categories)
        {
        ...
        public async Task<PaperSectionsViewModel?> RenderSectionsAsync(RenderSectionsRequest? request)
        {
            ...
            return new PaperSectionsViewModel
            {
                Settings = branding.Settings,
        ...
        public async Task<PaperDocumentViewModel?> RenderCanvasAsync(RenderCanvasRequest? request)
        {
            Branding? branding = await LoadBrandingAsync(request?.InstituteId);
            if (branding is null)
            {
            ...
            return new PaperDocumentViewModel
            {
        ...
        public async Task<int?> SaveAsync(SavePaperRequest? request)
        {
            Branding? branding = await LoadBrandingAsync(request?.InstituteId);
            if (branding is null)
            {
            ...
            // Title/section presence is validated by the controller before we get here.
        ...
        public async Task<PaperDocumentViewModel?> GetPrintModelAsync(int id)
        {
            ...
            if (!_tenant.IsSuperAdmin && test.InstituteId != _tenant.InstituteId)
            {
                return null;
            }
        ...
        {
            int? instituteId = _tenant.IsSuperAdmin ? requestedInstituteId : _tenant.InstituteId;
